...
mod tests {
    ...
    fn a_permalink_names_the_team_the_channel_is_in() {
        ...
        store
            .upsert_teams(&[team("t1", "curiosity"), team("t2", "sloclap")])
            .expect("teams");
        stored(&store, "c1", "t1");

        assert_eq!(
            permalink(&store, "https://chat.invalid", "p1").as_deref(),
            Some("https://chat.invalid/curiosity/pl/p1")
        );
        assert_eq!(
            permalink(&store, "https://chat.invalid/", "p1").as_deref(),
            Some("https://chat.invalid/curiosity/pl/p1")
    ...
    fn a_direct_message_borrows_a_team() {
        let store = Store::open_in_memory().expect("a store");
        store.upsert_teams(&[team("t2", "sloclap")]).expect("teams");
        stored(&store, "c1", "");
        assert_eq!(
            permalink(&store, "https://chat.invalid", "p1").as_deref(),
            Some("https://chat.invalid/sloclap/pl/p1")
        );
    }
