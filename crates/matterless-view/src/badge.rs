//! The taskbar button: its icon, the overlay badge, and flashing.
//!
//! Windows draws a small overlay in the corner of the taskbar icon
//! (`ITaskbarList3::SetOverlayIcon`) at the *small icon* size -- 16 physical
//! pixels at 100% scaling, 20 at 125%, 24 at 150%, 32 at 200%. The badge is
//! built at exactly that size: handing Windows a bigger bitmap looks like free
//! quality and is the opposite, since it then arrives through a filtered
//! downscale that softens every edge.
//!
//! **The number is set in Lato SemiBold, rasterised by GDI at the exact pixel
//! size it will be shown at.** A real font hinted onto the pixel grid is what
//! the official client gets from Skia -- and it is one typeface at every size,
//! where picking a different bitmap face per icon size means the same number
//! changes shape with the DPI.
//!
...
//!
//! Ported from the app's `taskbar.rs`, where Tauri supplied the last mile --
//! `set_overlay_icon` and `request_user_attention`. A window that owns its own
//! `HWND` has to reach `ITaskbarList3` and `FlashWindowEx` itself, which is the
...
/// the disc and being readable are different tests, and text that fails this
/// one is dropped rather than drawn.
const MIN_TEXT_HEIGHT: i32 = 7;

/// A square of straight (not premultiplied) RGBA, ready to become an icon.
...
/// Rasterises text with the bundled UI font, hinted onto the pixel grid.
#[cfg(windows)]
mod system_font {
    use super::Ink;
    use std::ffi::c_void;
    ...
    pub enum Edges {
        ...
        /// subpixels, and collapsing those three to one coverage byte -- which
        /// a white-on-any-colour badge has to -- leaves the digits mottled.
        Smooth,
        /// Hard pixels: sharper, at the cost of ragged curves. At 16 pixels the
        /// trade is arguable, which is why it is kept rather than deleted.
        #[allow(dead_code)]
        Hard,
    }
...
pub fn dot(size: u32) -> Overlay {
    let mut canvas = Canvas::new(size);
    // The palette's signal teal, so the badge belongs to the same family as
...
pub fn count(value: i64, size: u32) -> Overlay {
    badge(value, size, EDGES)
}

fn badge(value: i64, size: u32, edges: Edges) -> Overlay {
    let mut canvas = Canvas::new(size);
...
/// forces the whole number down to a smudge -- and once it is unreadable it
/// says less than a plain 99 does. Nothing is drawn if even that will not fit,
/// since the disc alone still says "something wants you".
...
/// What the taskbar should be showing, from what is unread.
///
/// One place, because the two states are exclusive and deciding them at each
/// call site is how a dot and a number come to be drawn over each other.
pub fn wanted(attention: i64, any_unread: bool, scale_factor: f64) -> Option<Overlay> {
    let size = size_for_scale(scale_factor);
    if attention > 0 {
        Some(count(attention, size))
    } else if any_unread {
        Some(dot(size))
    } else {
        None
...
/// What a screen reader is told the overlay means.
pub fn described(attention: i64, any_unread: bool) -> String {
    match (attention, any_unread) {
        (0, false) => String::new(),
        (0, true) => "Unread messages".to_string(),
        (1, _) => "1 message wants you".to_string(),
        (many, _) => format!("{many} messages want you"),
    }
}
...
mod tests {
    ...
    fn a_fresh_canvas_is_fully_transparent() {
        let canvas = Canvas::new(BASE_SIZE).into_overlay();
    ...
    fn the_dot_is_a_disc_that_fits_inside_the_icon() {
        let overlay = dot(BASE_SIZE);
        let painted = opaque_pixels(&overlay);
        ...
        assert_eq!(overlay.pixels[3], 0, "top-left corner must be transparent");
    }

    /// Nothing unread is nothing drawn: an overlay that never comes off says
    /// there is always something waiting, which is worse than none at all.
    #[test]
    fn a_quiet_account_carries_no_overlay() {
        assert_eq!(wanted(0, false, 1.0), None);
        assert!(wanted(0, true, 1.0).is_some(), "a dot for plain unread");
        assert!(wanted(3, true, 1.0).is_some(), "a number once asked");
        assert_ne!(
            wanted(0, true, 1.0),
            wanted(3, true, 1.0),
            "the two states have to look different"
        );
    }

    /// A count is louder than a dot: more of the icon is covered, which is what
    /// makes "you were named" visible without reading the number.
    #[test]
    fn a_count_takes_more_room_than_a_dot() {
        let quiet = opaque_pixels(&dot(BASE_SIZE));
        let asked = opaque_pixels(&count(1, BASE_SIZE));
        assert!(asked > quiet, "{asked} is no louder than {quiet}");
    }

    /// The description is what a screen reader reads out, so it has to say
    /// something different in each state and nothing when there is no overlay.
    #[test]
    fn the_description_matches_the_overlay() {
        assert!(described(0, false).is_empty());
        assert_eq!(described(0, true), "Unread messages");
        assert_eq!(described(1, true), "1 message wants you");
        assert_eq!(described(12, true), "12 messages want you");
    }
    ...
    mod on_windows {
        ...
        /// Every count that can be drawn has to stay inside the disc, at every
        /// DPI: a digit over the rim reads as a smear rather than a number.
        #[test]
        fn a_number_fits_its_disc_at_every_size() {
            for scale in [1.0, 1.25, 1.5, 2.0] {
                let size = size_for_scale(scale);
                for value in [1, 9, 10, 42, 99, 1000] {
                    let Some(ink) = drawn_form(value, size, Edges::Smooth) else {
                        // Nothing that fits is a legitimate answer: the disc
                        // alone still says something wants you.
                        continue;
                    };
                    assert!(
                        ink.fits(size as f32 * BADGE_RADIUS),
                        "{value} at {size} reaches past the rim"
                    );
                    assert!(ink.height <= tallest_text(size));
                }
            }
        }

        /// Three digits never appear: a thousand mentions is drawn as 99.
        #[test]
        fn a_huge_count_is_clamped_rather_than_shrunk() {
            let big = count(4321, 32);
