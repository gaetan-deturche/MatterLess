...
fn conversation() -> Vec<Row> {
    vec![
        ...
        Row::Post {
            post: post("simon.odwyer", vec![para(vec![text("Yo!")])]),
        },
        Row::Continuation {
            post: post(
                "simon.odwyer",
                vec![para(vec![text(
                    "I would like to raise this limit on swarm please, to fifty or perhaps a \
                     ...
                     directory. Do you have access to that in the tools team?",
                )])],
            ),
        },
        Row::Post {
            post: post(
                ...
                vec![
                    para(vec![
                        text("ah that is "),
                        Node::Strong {
                            children: vec![text("a good question")],
                        },
                        text(" -- ask "),
                        Node::UserMention {
                        ...
                        text(", he installed it"),
                    ]),
                    Node::List {
                        ordered: false,
                        items: vec![
                            vec![para(vec![text("we have access to the P4 server")])],
                            vec![para(vec![text("but not to where the plugin lives")])],
                        ],
                    },
                    Node::CodeBlock {
