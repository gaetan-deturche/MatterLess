...
mod tests {
    ...
    fn a_kept_shaping_draws_exactly_where_a_fresh_one_would() {
        ...
        let fresh = Painter::new().run(&mut fonts, "Curiosity | Progression", 31.0, 17.0, run);
        let mut painter = Painter::new();
        // The first call shapes it, the second can only have come from the
        // store -- and both are asked for in the same place.
        let first = painter.run(&mut fonts, "Curiosity | Progression", 31.0, 17.0, run);
        let again = painter.run(&mut fonts, "Curiosity | Progression", 31.0, 17.0, run);
    ...
    fn a_sweep_does_not_lose_a_label_that_is_still_in_use() {
        ...
        let mut painter = Painter::new();
        let run = Run::label(f32::MAX);
        let kept = painter.run(&mut fonts, "Curiosity | Dev", 4.0, 9.0, run);
        ...
        let after = painter.run(&mut fonts, "Curiosity | Dev", 4.0, 9.0, run);
        assert!(
            same(&kept, &after),
